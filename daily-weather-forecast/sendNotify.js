...
let BARK_PUSH = '';
//BARK_APP推送设备列表, 分号分隔(环境变量名 BARK_USER_DEVICES_ID_LIST)
let BARK_USER_DEVICES_ID_LIST = [];
...
if (process.env.BARK_URL && process.env.BARK_USER_DEVICES_ID_LIST) {
  if (
    process.env.BARK_URL.indexOf('https') > -1 ||
    process.env.BARK_URL.indexOf('http') > -1
  ) {
    //兼容BARK自建用户
    BARK_PUSH = process.env.BARK_URL;
    BARK_USER_DEVICES_ID_LIST = process.env.BARK_USER_DEVICES_ID_LIST.split(';');
  }
  ...
  return new Promise((resolve) => {
    if (BARK_PUSH) {
      const urlParams = {
        ...
        sound: BARK_SOUND,
        group: BARK_GROUP,
        device_keys: BARK_USER_DEVICES_ID_LIST,
        ...params,
      };
      const options = {
        url: `${BARK_PUSH}/push`,
